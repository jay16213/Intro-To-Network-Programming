...
int main(int argc,char *argv[]){
	...
	if(argc != 3){
		printf("Error: ./test <path to program's directory> <port>\n");
		return 0;
	}
	...
	/* diff */
	diff("0/testfile2","2/testfile2","\tTest3 : same time upload PASS1.\n","\tTest3 : same time upload FAIL1.\n",&score_same);
	diff("1/testfile3","2/testfile3","\tTest3 : same time upload PASS2.\n","\tTest3 : same time upload FAIL2.\n",&score_same);
	printf("\n");
	/********************* Test3 finish *********************/
	...
	sleep(10);
        ...
        ("0/testfile4","3/testfile4","\tTest4 : Sleep PASS3.\n","\tTest4 : Sleep FAIL3.\n",&score_sleep);
	printf("\n");
	/********************* Test4 finish *********************/

	/********************* Test5 Exit *********************/
	printf("Test5 : Exit, and delete all file.\n");
	PAUSE
	int status;
	for(i = 0; i < 4 ; i++){
	...
	printf("Test5 : %d\n\n",score_exit);
	printf("\t--------------------\n");
	printf("Total : %d\n",score_onefile+score_login+score_same+score_sleep+score_exit);
}
...
void openServer(pid_t *server,char *port){
        ...
        switch(*server = fork()){
                ...
                case 0:
                        close(pipe_server[0]);
                        dup2(pipe_server[1],STDOUT_FILENO);
                        close(pipe_server[1]);
